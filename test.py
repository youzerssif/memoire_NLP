...
#         vals = cosine_similarity(tfidf[-item], tfidf)
#         idx=int(vals.argsort()[0][-2])
#         flat = vals.flatten()
#         flat.sort()
#         req_tfidf = flat[-2]
#         if(req_tfidf==0):
...
# 
    TfidfVec = TfidfVectorizer(tokenizer=LemNormalize, stop_words=final_stopwords_list)
    tfidf = TfidfVec.fit_transform(sent_tokens)
    # print('TEST TF-IDF ==========>>', TfidfVec.inverse_transform(tfidf))
    vals = cosine_similarity(tfidf[-1], tfidf)
    idx=int(vals.argsort()[0][-2])
    flat = vals.flatten()
    flat.sort()
    req_tfidf = flat[-2]
    # print('TEST SIMILARITE COSINUS =========>>', req_tfidf)
    if(req_tfidf==0):
        robo_response=robo_response+"I am sorry! I don't understand you"
        return robo_response
    else:
...
flag=True
print("RoboUVCI: My name is RoboUVCI. I will answer your queries about Chatbots. If you want to exit, type Bye!")
...
while(flag==True):
    ...
    if(user_response!='bye'):
        if(user_response=='thanks' or user_response=='thank you' ):
            flag=False
            print("RoboUVCI: You are welcome..")
        else:
            if(greeting(user_response)!=None):
                print("RoboUVCI: "+greeting(user_response))
            else:
